...
function Title({ setMessages }: Props) {
    ...
    await axios
      ...
      .then((res) => {
        if (res.status == 200) {
          ...
          console.error("Something went wrong with the API request");
        }
      })
      .catch((err) => {
  ...
  return (
    <div className="flex justify-between items-center p-4 w-full bg-gray-900 text-white font-bold shadow">
      <div className="italic">VoiceGPT</div>
      <div className="italic text-sm font-light">(Use Chrome for best experience)</div>
      <button
